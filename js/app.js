...
(function () {
  ...
  function renderC4Battle(){const c=save.chapter4,b=c.battle=normalizeChapter4Battle(c.battle||createChapter4Battle()),intent=c4IntentText(b),reason=c4PurifyReason(b);
    ...
    const celesteActions=[['bait','Горькая приманка','Защитить следующий положительный эффект.','Команда',b.teamUsed?'Действие использовано':''],['dose','Формула Селесты',C4_RECIPES[c.recipeChoice].desc,`${b.potionCharges} доз.`,b.teamUsed?'Действие использовано':b.potionCharges<1?'Нет дозы':''],['cleanse_toxin','Перегнать остаток',b.toxin?'Снять токсин смеси.':'Создать ещё одну приманку.','Команда',b.teamUsed?'Действие использовано':'']];
    const lioraActions=[['aegis','Эгида','Дать Щит обеим защищаемым целям.','Команда',b.teamUsed?'Действие использовано':''],['light','Луч Первого Света','Снять 1 слой Покрова.','Команда',b.teamUsed?'Действие использовано':'']];
  ...
  if (globalThis.__MAGIC_RPG_TEST__) {
    globalThis.__magicTest={
      ...
      createChapter4State,normalizeChapter4State,ensureChapter4State,createChapter4Battle,normalizeChapter4Battle,
      setSave(value,screen='home'){save=ensureUX6State(ensureChapter4State(ensureUX5State(value)));currentScreen=screen;},
      render(){renderGame();},renderStart(){renderStart();},getSave(){return save;},getHtml(){return app.innerHTML;},canC2Purify,c2PurifyReason,incidentFinalReason
    };
  }
